...
Describe 'ProjectWorld generated-artifact manifest lifecycle' {
    BeforeEach {
        ...
        function script:NewCandidate {
            ...
            if ([string]::IsNullOrWhiteSpace($GeneratorFingerprint)) {
                $producerId = if ($Layer -eq 'presentation') { 'presentation:v1' } else { 'map:v1' }
                $GeneratorFingerprint = Get-ProjectWorldGeneratorFingerprint `
    ...
    It 'creates a metadata-only fingerprint migration candidate' {
        Enroll | Out-Null
        $activeSet = Read-ProjectWorldActiveSet -ManifestRoot $manifestRoot
        $prior = $activeSet.Manifests[$mapScopeId] | ConvertTo-Json -Depth 8 | ConvertFrom-Json
        $prior.owning_layer = 'roads'
        $prior | Add-Member -NotePropertyName layer_contract -NotePropertyValue ([pscustomobject]@{
            generator_id = 'project_road_mesh'
            generator_version = 1
            semantic_outputs = @([pscustomobject]@{ unit_id = 'cell-1'; sha256 = 'e' * 64 })
        })
        $candidate = New-ProjectWorldFingerprintMigrationCandidate `
            -PriorManifest $prior -Generation 2 -OperationId ('a4' * 16) `
            -GeneratorFingerprint ('f' * 64)

        foreach ($field in @(
            'scope_id', 'owning_layer', 'input_identity', 'artifacts',
            'consumer_references', 'layer_contract')) {
            ($candidate.$field | ConvertTo-Json -Depth 8 -Compress) | Should -Be `
                ($prior.$field | ConvertTo-Json -Depth 8 -Compress)
        }
        $candidate.generation | Should -Be 2
        $candidate.accepted_operation_id | Should -Be ('a4' * 16)
...
Describe 'Production enrollment refuses the unattended path (C8 operator control)' {
    # The irreversible operation guards ITSELF. A Claude/Codex permission rule
    # only covers the tools someone remembered to configure; a third agent, a
    # CI job, or a plain shell would bypass it. TestData enrollment stays
    # unattended-legal because the E2E validator and the lifecycle integration
    # test bootstrap synthetic scopes that way.

    BeforeAll {
        $script:Wrapper = (Resolve-Path (Join-Path $PSScriptRoot '..\realize_canonical_world.ps1')).Path
        $script:RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..\..')).Path

        # Defined in BeforeAll, not the Describe body: Pester 5 runs
        # Describe at discovery time, so a function declared there is not
        # in scope inside It.
        function New-CompileFixture {
            param([string]$Owner)
            $coveragePath = Join-Path $script:Fixture 'canonical/coverage.json'
            Set-Content -LiteralPath $coveragePath -Encoding UTF8 -NoNewline `
                -Value (@{ world_data_plugin = $Owner } | ConvertTo-Json -Compress)
            $info = Get-Item -LiteralPath $coveragePath
            $hash = (Get-FileHash -LiteralPath $coveragePath -Algorithm SHA256).Hash.ToLowerInvariant()
            $receipt = @{
                outputs = @(@{
            ...
            $receiptPath = Join-Path $script:Fixture 'compile_result.json'
            Set-Content -LiteralPath $receiptPath -Encoding UTF8 `
                -Value ($receipt | ConvertTo-Json -Depth 6)
            return $receiptPath
        }
    }

    BeforeEach {
        $script:Fixture = Join-Path ([System.IO.Path]::GetTempPath()) ("c8_" + [guid]::NewGuid().ToString("N"))
        New-Item -ItemType Directory -Path (Join-Path $script:Fixture 'canonical') -Force | Out-Null
    }

    AfterEach {
        Remove-Item $script:Fixture -Recurse -Force -ErrorAction SilentlyContinue
    }

    # C8 revised 2026-08-20: the authority boundary is the MANIFEST ROOT, not the
    # owning plugin. Enrolling into the durable root grants production authority
    # and stays operator-only; enrolling into a transient candidate root touches
    # no durable authority and must remain unattended-legal, because the territory
    # Matrix and every future layer admission enroll into their own sandbox root.
    It 'REFUSES ProjectWorldData enrollment into the DURABLE manifest root with -NonInteractive' {
        ...
        # Real profiles: the refusal is now evaluated after the manifest root is
        # resolved (still before any lock, snapshot, or journal), so placeholder
        # paths would fail earlier on profile existence and never reach the guard.
        ...
        $output = & powershell.exe -NoProfile -ExecutionPolicy Bypass -Command `
            "& '$script:Wrapper' -CompileResult '$receipt' -Mode Apply -EnrollManifests -NonInteractive -Map '/ProjectWorldData/Generated/Territory/L_ProjectWorldKazanTerritory' -AuthoredOverlayProfile '$authored' -PresentationProfile '$presentation' -ManifestRoot '$durable'" 2>&1
        $LASTEXITCODE | Should -Not -Be 0
