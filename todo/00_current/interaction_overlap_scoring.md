...
**Status:** Plan first, no code yet. Revised 2026-04-24 after design review.
...
4. Stay inside `ProjectInteraction`'s current module graph (no new plugin
   ...
   abstractions, or ProjectCore interfaces.** Private helper structs and
   free functions inside `ProjectInteraction`'s `.cpp` anonymous namespace
...
```
Tick (every N frames, respects TraceFrameInterval)
  |
  v
...
still throttled by `TraceFrameInterval = 6` (about 10 Hz).
...
| Property | Type | Default (Slice 1) | Default (Slice 2+) | Meaning |
|---|---|---:|---:|---|
| `InteractionRadius` | float | 300.0 | 300.0 | Sphere radius around the pawn. Replaces semantic role of `TraceDistance`. |
| `ShortCircuitRadius` | float | 60.0 | 60.0 | Within this distance, bypass LOS gate only (still subject to aim gate). |
| `MinAimDot` | float | 0.2 | 0.2 | Minimum dot(forward, to-target). Always enforced, cannot be bypassed. |
| `AimWeight` | float | 1.0 | 1.0 | Scoring weight for aim alignment. |
| `DistanceWeight` | float | 0.25 | 0.25 | Scoring weight for proximity. |
| `PriorityWeight` | float | **0.0** | 0.5 | Disabled in slice 1; enabled in slice 2. |
| `FocusSwitchHysteresis` | float | -- | 0.10 | Slice 2 jitter guard. Keep current focus unless new score beats current by > 10 %. |

Existing:
- `TraceDistance` (deprecated; keep for backward compat one release, then
  remove -- see slice 4).
- `TraceChannel` kept, same semantics (used ONLY by the LOS line trace,
  NOT by the overlap gather).
- `TraceRadius` kept but now unused. Plan to remove in slice 4 (was
  originally intended to be sphere-trace radius; we replaced it with a
  cleaner `InteractionRadius` for the new overlap-based design).
- `TraceFrameInterval` kept, unchanged.
...
All helpers sit next to the existing anonymous namespace at
[InteractionComponent.cpp:17-245](../../Plugins/Gameplay/ProjectInteraction/Source/ProjectInteraction/Private/InteractionComponent.cpp#L17-L245).
`UpdateTrace` becomes a short orchestrator. No new files. Expected net LOC
delta: +160 to +200. See the "LOC" row under Risks for the pre-work
extraction that keeps the TU under 1000 lines.
...
matches `TraceFrameInterval` so shapes do not pile up):
- overlap sphere at ViewOrigin
- one line per candidate: red if aim-rejected, yellow if LOS-rejected,
...
Before slice 1, extract the existing 245-line anonymous namespace block
(`CollectAttachmentHierarchy`, `GatherInteractableComponents`,
`SelectBestInteractableComponent`, `ResolveFocusFromComponents`,
`ResolveInteractionExecutionSpecFromComponents`,
`ExecuteInteractionViaComponents`, helpers) into a sibling
`InteractionComponent_Helpers.cpp` TU (cross-TU split pattern per
`canonical.md` section 6). Zero behavior change, zero header change.
...
Slice 1 changes ONLY `UpdateTrace`'s body and introduces the pure
resolver + `FInteractionCandidate` in the anonymous namespace. No new
subsystem, no collision config migration, no Lyra-style Interaction
channel.

### Slice 2 -- priority scoring + hysteresis

- Flip `PriorityWeight` default from 0 to 0.5. Enable the
  `PriorityScore` term in the unified scoring function.
- Introduce `FocusSwitchHysteresis = 0.10` and apply inside
  `UpdateTrace` before calling `SetFocusedActor`:
  keep current focus unless new score beats current by > 10 %.
...
- Remove dead `TraceRadius` field after one release with deprecation log.
- Rename `TraceDistance` -> `InteractionRadius` on property; keep
  `CoreRedirects` for the property for one release per
  [docs/editor/class_migration.md](../../docs/editor/class_migration.md).
- **Optional:** add dedicated `Interaction` trace channel in
  `Config/DefaultEngine.ini` if slice 2/3 profiling shows the interface
  filter is meaningfully expensive. Interactable meshes overlap
  `Interaction`, walls keep blocking `Visibility`. Not required unless
  metrics say so.
- Delete `TODO(Interaction): dynamic-camera retry` comments if any.

---

## Landing rule (enforced, not negotiable)

Do NOT land "full scoring + hysteresis + server parity" in one PR.

Land exactly this first:
...
Slice 2 (priority + hysteresis) is a separate PR. Mixing them hides
bugs -- for example, hysteresis can mask a scoring regression, and
enabling priority weight can mask a filter regression.

Slice 3 (server parity) is a separate PR. Test it in dedicated-server
PIE mode or a real `AlisServer` build. Do NOT rely on listen-server
PIE for the validation -- listen server shares the client's
`PlayerCameraManager` state and will mask `GetActorEyesViewPoint`
regressions.

Slice 4 (cleanup) is a separate PR. Dedicated `Interaction` trace
channel stays optional; only add it if slice 2/3 profiling or content
noise proves the need.
...
| Perf regression | Throttled by `TraceFrameInterval=6`; overlap + N short line traces, N typically <= 5. Epic forum consensus: negligible vs single line trace. |
...
| Scoring picks behind-head target when all aim dots are negative | `MinAimDot = 0.20` is always enforced in the aim gate (cannot be bypassed by `ShortCircuitRadius`). |
...
| Net code change in InteractionComponent pushes file over 1000 LOC | Current 884 + estimated +200 = ~1084. Mitigation: **Slice 0** extracts the existing 245-line anonymous namespace into `InteractionComponent_Helpers.cpp` sibling TU (cross-TU split pattern per `canonical.md` section 6). Single commit, zero behavior change. |
...
### Required automated test (lands with or immediately after slice 1)

One resolver regression test in `ProjectIntegrationTests` exercising
the pure resolver directly (no `UpdateTrace`, no `SetFocusedActor`).
Tagged `[Fast][Integration][Interaction]`. Mandatory before slice 2
opens; can ship in the same slice 1 PR or a follow-up landed before
priority/hysteresis work begins.
...
- **2026-04-25 revision 3a (this file)** -- approval-cleanup pass:
  ...
  returns linear distance (not squared), so the helper drops the
  `const_cast` and the misnamed `DistanceSquared` local; squared-
  distance API is `GetSquaredDistanceToCollision`. Note added inline
  to prevent future mix-up.
